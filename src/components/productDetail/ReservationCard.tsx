...
export default function ReservationCard({ accommodationData }: Props) {
	...
	const [paymentInfo, setPaymentInfo] = useState({
		room: query.get("room") || accommodationData.rooms[0].max_capacity,
	...
	const handleCart = async () => {
		await fetch("/api/carts", {
			...
			body: JSON.stringify({
				room_id: id,
				capacity: paymentInfo.guest,
				start_date: paymentInfo.startDate,
				end_date: paymentInfo.endDate,
	...
	const totalPrice = () => {
		...
		const selectedRoom = accommodationData.rooms.find((roomItem: Room) => roomItem.max_capacity === paymentInfo.room);
			...
			<div className="h-min p-6 border rounded-lg sticky top-[200px]">
				...
				<div className="border rounded mb-4">
					...
					<div ref={dateOpenRef} className="relative border-b">
						...
						{isDate && (
							<div ref={dateRef} className="z-10 absolute -top-1/2 -right-1/2 bg-white border shadow-md p-4">
								<DateRange
									locale={ko}
									ranges={[date]}
